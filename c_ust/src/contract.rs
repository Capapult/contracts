...
use crate::state::{balances, balances_read, token_info, token_info_read, MinterData, TokenInfo,store_config,read_config, Config};
...
pub fn register_minter<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    env: Env,
    minter_addr: HumanAddr,
) -> StdResult<HandleResponse> {
    ...
    let mint = Some(MinterData {
            minter: deps.api.canonical_address(&minter_addr)?,
            cap: None,
        });
